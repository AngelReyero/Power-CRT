...
def main(args):
    ...
    for n in n_samples:
        ...
        n_components_Z = 5
        method_distill_Z="pca"
        model_distillation=get_base_model("SL", s, n_jobs=1)
        # --- Run CRT ---
        results = CRT_comparison(
            ...
            Z,
            T_list,
            distill_Y=lambda Y,Z: distill_Y(Y, Z, model_distillation),
            distill_Z=lambda Z: distill_Z(Z, method=method_distill_Z, n_components=n_components_Z, random_state=s),
            n_jobs=n_jobs,
    ...
    csv_path = os.path.abspath(
        os.path.join(
            ...
            f"../../results/csv/sim_100D/simulation_{setting}_model{args.model}_seed{s}.csv",
        )
    )
